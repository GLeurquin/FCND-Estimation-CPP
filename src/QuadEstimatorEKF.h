...
public:
  ...
  QuadEstimatorEKF(string config,
                   string name);
  ...
  VectorXf PredictState(VectorXf curState,
                        float    dt,
                        V3F      accel,
                        V3F      gyro);
  MatrixXf GetGPrime(float dt,
                     V3F   accel);
  MatrixXf GetRbgPrime(float roll,
                       float pitch,
                       float yaw);

  virtual void UpdateFromIMU(V3F accel,
                             V3F gyro);
  virtual void UpdateFromGPS(V3F pos,
                             V3F vel);
  ...
  float accelPitch, accelRoll; // raw pitch/roll angles as calculated from last
                               // accelerometer.. purely for graphing.
  V3F accelG;
  V3F lastGyro;

  // generic EKF update
  // z: measurement
  // H: Jacobian of observation function evaluated at the current estimated
  // state
  ...
  virtual bool          GetData(const string& name,
                                float       & ret) const;
  virtual vector<string>GetFields() const;
  string _name;

  // error vs ground truth (trueError = estimated-actual)
  virtual void UpdateTrueError(V3F                   truePos,
                               V3F                   trueVel,
                               SLR::Quaternion<float>trueAtt);
  VectorXf trueError;
