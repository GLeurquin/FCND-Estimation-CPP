...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // The integrated (predicted) value is then updated in a complementary filter
  // style with attitude information from accelerometers
  //
  // Implement a better integration method that uses the current attitude
  // estimate (rollEst, pitchEst and ekfState(6))
  // to integrate the body rates into new Euler angles.
  //
  // HINTS:
  //  - there are several ways to go about this, including:
  //    1) create a rotation matrix based on your current Euler angles,
  // integrate that, convert back to Euler angles
  //    OR
  //    2) use the Quaternion<float> class, which has a handy FromEuler123_RPY
  // function for creating a quaternion from Euler Roll/PitchYaw
  //       (Quaternion<float> also has a IntegrateBodyRate function, though this
  // uses quaternions, not Euler angles)
  ...
  V3D   euler          = attitude.ToEulerRPY();
  float predictedRoll  = euler.x;
  float predictedPitch = euler.y;
  ekfState(6) = euler.z; // Yaw
  ...
  rollEst = attitudeTau / (attitudeTau + dtIMU) * (predictedRoll) + dtIMU /
            (attitudeTau + dtIMU) * accelRoll;
...
void QuadEstimatorEKF::UpdateTrueError(V3F              truePos,
                                       V3F              trueVel,
                                       Quaternion<float>trueAtt)
{
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState,
                                        float    dt,
                                        V3F      accel,
                                        V3F      gyro)
{
  ...
  //   accel: acceleration of the vehicle, in body frame, *not including
  // gravity* [m/s2]
  ...
  // HINTS
  // - dt is the time duration for which you should predict. It will be very
  // short (on the order of 1ms)
  //   so simplistic integration methods are fine here
  // - we've created an Attitude Quaternion for you from the current state. Use
  //   attitude.Rotate_BtoI(<V3F>) to rotate a vector from body frame to
  // inertial frame
  // - the yaw integral is already done in the IMU update. Be sure not to
  // integrate it again here
  ...
  // Convert accel in body frame to inertial frame
  V3F accelerationInInertialFrame = attitude.Rotate_BtoI(accel);

  // Remove the acceleration
  accelerationInInertialFrame -= V3F(0, 0, 9.81f);
  ...
  // X velocity
  predictedState(3) = curState(3) + dt * accelerationInInertialFrame.x;

  // Y velocity
  predictedState(4) = curState(4) + dt * accelerationInInertialFrame.y;

  // Z velocity
  predictedState(5) = curState(5) + dt * accelerationInInertialFrame.z;

  // Yaw, already integrated in the IMU, so don't change it here
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  // Return the partial derivative of the Rbg rotation matrix with respect to
  // yaw. We call this RbgPrime.
  // INPUTS:
  //   roll, pitch, yaw: Euler angles at which to calculate RbgPrime
  //
  // OUTPUT:
  //   return the 3x3 matrix representing the partial derivative at the given
  // point

  // HINTS
  // - this is just a matter of putting the right sin() and cos() functions in
  // the right place.
  //   make sure you write clear code and triple-check your math
  // - You can also do some numerical partial derivatives in a unit test scheme
  // to check
...
MatrixXf QuadEstimatorEKF::GetGPrime(float dt, V3F accel) {
  // Create an empty Jacobian, currently simply set to identity
  MatrixXf gPrime(QUAD_EKF_NUM_STATES, QUAD_EKF_NUM_STATES);

  // Initialize g prime to the identity matrix
  gPrime.setIdentity();
  ...
  V3F RbgPrime0(RbgPrime(0, 0), RbgPrime(0, 1), RbgPrime(0, 2));
  V3F RbgPrime1(RbgPrime(1, 0), RbgPrime(1, 1), RbgPrime(1, 2));
  V3F RbgPrime2(RbgPrime(2, 0), RbgPrime(2, 1), RbgPrime(2, 2));
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  // Predict the current covariance forward by dt using the current
  // accelerations and body rates as input.
  // INPUTS:
  //   dt: time step to predict forward by [s]
  //   accel: acceleration of the vehicle, in body frame, *not including
  // gravity* [m/s2]
  //   gyro: body rates of the vehicle, in body frame [rad/s]
  //   state (member variable): current state (state at the beginning of this
  // prediction)
  //
  // OUTPUT:
  ...
  //
  // - you may find the current estimated attitude in variables rollEst,
  // pitchEst, state(6).
  //
  // - use the class MatrixXf for matrices. To create a 3x5 matrix A, use
  // MatrixXf A(3,5).
  //
  // - the transition model covariance, Q, is loaded up from a parameter file in
  // member variable Q
  //
  // - This is unfortunately a messy step. Try to split this up into clear,
  // manageable steps:
  //   1) Calculate the necessary helper matrices, building up the transition
  // jacobian
  //   2) Once all the matrices are there, write the equation to update cov.
  //
  // - if you want to transpose a matrix in-place, use A.transposeInPlace(), not
  // A = A.transpose()
  //
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  MatrixXf gPrime = GetGPrime(dt, accel);

  // Predict the covariance matrix forward
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  // Set current state of measured value (h)
  zFromX(0) = ekfState(6);
...
void QuadEstimatorEKF::Update(VectorXf& z,
                              MatrixXf& H,
                              MatrixXf& R,
                              VectorXf& zFromX)
{
...
// Calculate the condition number of the EKF ovariance matrix (useful for
// numerical diagnostics)
// The condition number provides a measure of how similar the magnitudes of the
// error metric beliefs
// about the different states are. If the magnitudes are very far apart,
// numerical issues will start to come up.
...
#define GETTER_HELPER(A, B) \
  if (SLR::ToUpper(rightPart) == SLR::ToUpper(A)) { ret = (B); return true; }
